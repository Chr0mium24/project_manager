...
import { applyManagedTask } from "../lib/managed-task.mjs";
...
function main() {
  ...
  if (!args.contentRepo || !args.projectSlug || !args.taskSlug) {
    throw new Error("usage: node scripts/apply-managed-task.mjs --content-repo <path> --project <slug> --task <slug>");
  }

  const validationRoot = process.cwd();
  const contentRepoRoot = path.resolve(validationRoot, args.contentRepo);
  const result = applyManagedTask(validationRoot, {
    contentRepoRoot,
