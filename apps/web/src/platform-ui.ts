...
const PLATFORM_UI_STYLES = [
  ...
  ".overview-card { border: 1px solid var(--line); background: var(--panel-soft); padding: 16px; }",
  ".overview-card strong { display: block; font-size: 0.76rem; text-transform: uppercase; letter-spacing: 0.14em; color: var(--muted); }",
  ".overview-card span { display: block; margin-top: 10px; font-size: 1rem; font-weight: 700; }",
  ".overview-card p { margin: 10px 0 0; color: var(--muted); line-height: 1.6; }",
  ".overview-links { margin-top: 16px; }",
  ...
  "@media (max-width: 900px) { .hero-grid, .meta-grid, .workspace-grid, .version-grid, .action-row, .route-nav, .overview-grid { grid-template-columns: 1fr; } .shell { width: min(100% - 20px, 1320px); margin-top: 20px; } .panel, .service-card { padding: 18px; } .version-diff { border-left: 0; border-top: 1px solid var(--line); padding-left: 0; padding-top: 16px; } }"
].join("\n");
...
function renderMainContent(): string {
  return [
    ...
    "            <div class=\"panel-head\"><h2>New AI Task</h2></div>",
